...
export const XSOLLA_STARTER_RECEIPT_V2_KEY_PREFIX = "xss2_";
export const XSOLLA_STARTER_RECEIPT_V2_SCHEMA_VERSION = 2;
...
export function getXsollaStarterReceiptV2Key(transactionId) {
    ...
    return XSOLLA_STARTER_RECEIPT_V2_KEY_PREFIX + createHash("sha256")
        .update(transaction, "utf8")
        .digest("base64url");
}

export function serializeXsollaStarterReceiptV2(receipt = {}) {
    const {
        transactionId,
        provider,
    ...
    getXsollaStarterReceiptV2Key(transactionId);
    ...
    if (provider !== "xsolla" || providerTransactionId !== transactionId ||
        ...
        throw new TypeError("Xsolla Starter v2 provider identity is invalid.");
    }
    ...
    try {
        ...
        rewardPlan = getStarterRewardPlan(xsollaSku, rewardPlanVersion);
    } catch {
        throw new TypeError("Xsolla Starter v2 plan version is invalid.");
    }
    if (productPlan.productId !== productId || productPlan.productType !== productType ||
        ...
        promotionPolicy !== "disabled" || rewardPlan.productId !== productId ||
        rewardPlanHash !== rewardPlan.rewardPlanHash || !Array.isArray(rewards) ||
        JSON.stringify(rewards) !== JSON.stringify(rewardPlan.rewards)) {
        throw new TypeError("Xsolla Starter v2 immutable contract is invalid.");
    }

    return JSON.stringify({
        schemaVersion: XSOLLA_STARTER_RECEIPT_V2_SCHEMA_VERSION,
        transactionId,
        notificationType,
...
export function createPlayFabXsollaStarterReceiptV2Store({
    titleId,
    secretKey,
    timeoutMs = 8000,
    ...
    return async function persistXsollaStarterReceiptV2(receipt) {
        if (!configured) {
            throw new Error("PlayFab Xsolla Starter v2 persistence is not configured.");
        }
        ...
        const legacyKey = getXsollaStarterReceiptKey(receipt.transactionId);
        const legacyValue = serializeXsollaStarterReceipt(receipt);
        const keys = [key, legacyKey];
        const existing = await read(receipt.playFabId, keys);
        if (existing.get(key) !== null && existing.get(key) !== value) {
            throw new Error("Immutable Xsolla Starter v2 receipt conflict.");
        }
        if (existing.get(legacyKey) !== null && existing.get(legacyKey) !== legacyValue) {
            throw new Error("Immutable Xsolla Starter legacy receipt conflict.");
        }
        ...
        if (Object.keys(data).length > 0) {
            ...
            if (readback.get(key) !== value || readback.get(legacyKey) !== legacyValue) {
                throw new Error("PlayFab Xsolla Starter v2 receipt readback mismatch.");
            }
        }
